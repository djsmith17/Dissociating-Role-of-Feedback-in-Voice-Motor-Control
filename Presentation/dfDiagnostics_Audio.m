...
dirs.RecFileDir    = fullfile(dirs.RecData, expParam.subject, expParam.run); %Where to save Recording (Raw data) 
dirs.RecWaveDir    = fullfile(dirs.RecFileDir, 'wavFiles');
dirs.SavResultsDir = fullfile(dirs.RecData, expParam.subject, expParam.run); %Where to save analyzed results 
...
if CnD == 1
    ...
    %experimentation
    dirs.InflaRespFile = 'W:\Experiments\Dissociating-Role-of-Feedback-in-Voice-Motor-Control\null\null_AveInflaResp.mat';
    try
