...
% edited by Liz Heller Murray 5/21/2015 - will now only change if there are
% 2 correct in a row
...
if ~exist( answer{ 1 } , 'dir' )
    mkdir( answer{ 1 } ) ;
      ...
      if ~strcmp(overwrite,'yes') & ~strcmp(overwrite,'YES') & ~strcmp(overwrite,'Yes') 
          'If you want to overwrite, type YES'
          return;
      end
end
...
for Phase = 1 : 2 %phase 1 plays first then phase 2
 ...
 sound(.05*soundwav*10^(-6/20),fs); %65 dB on MOTU equipment
   ...
   match(Trial) = isequal(isDiff,(response(Trial))); % if it is a catch trial, then isDiff will be zero, if they respond "same" (0), then it will be a match, if if it is not a catch trial, isDiff will be 1 and if they response different it will be a match, otherwise it will not be a match
%
