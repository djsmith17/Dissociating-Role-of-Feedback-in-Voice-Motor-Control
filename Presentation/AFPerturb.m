...
for i = 1:numTrial
    St   = round(s.Rate*(1.7 + (2.1-1.7)*rand)); %Hardset
    pLen = round(s.Rate*(0.7 + (1.1-0.7)*rand)); %Hardset    
    Sp   = St+pLen;    
    span = St:Sp; 

    sig  = zeros(trialLen,1);
    if trialType(i) == 1
        sig(span) = 0; %in the SFPerturb function this is 3. In AFPerturb, we dont need anything here
    end
