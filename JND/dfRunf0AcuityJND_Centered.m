...
%dfRunf0Acuity_Centered() is an experimental paradigm for testing perceptual
%acuity to pitch using a AX method. This program uses a previously
%generated set of speech tokens to be presented to the participant. On each
%trial of this task, the participant listens to two tokens of their own
%voice, and must decide if the pitch of the two tokens is the same or 
...
numlines = 1;
defaultanswer = {'null','fAC1', 'GT1', 'female'};
...
while (UD.stop == 0) && tr < UD.totalTrials
    ...
    [trialType, correct] = accuLogic(conVar, response);
...
function [trialType, correct] = accuLogic(conVar, response)

if conVar == 1
    if response == 1 
        trialType = 1; %Correct Diff
        correct   = 1; %Correct
    else
        trialType = 2; %Incorrect Diff
        correct   = 0; %Incorrect
    end
elseif conVar == 0        
    if response == 0 
