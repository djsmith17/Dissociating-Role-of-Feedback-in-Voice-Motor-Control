function drawMeanSubjf0Resp_Offset(poolRes, targPixDim, plotFolder, fLabel, fStat, varargin)
...
if presFlag == 1
    axes(ax2)
    pL = plot(timeP, sensorP.OF.mean, 'color', pressureC, 'LineStyle', ':', 'LineWidth', lineThick);
...
nC = shadedErrorBar(time, contf0.OF.mean, contf0.OF.NCI, 'lineprops', 'k', 'transparent', 1);
...
for ii = 1:numCond
    nM = shadedErrorBar(time, pertf0M{ii}.OF.mean, pertf0M{ii}.OF.NCI, 'lineprops', condColors(ii), 'transparent', 1);
    ...
    if drawHead == 1
        nM = shadedErrorBar(time, pertf0H{ii}.OF.mean, pertf0H{ii}.OF.NCI, 'lineprops', condColors(ii), 'transparent', 1);
...
plTitle = [pltName '_OffsetZoom.jpg'];
saveFileName = fullfile(plotFolder, plTitle);
export_fig(saveFileName, '-r300')
end
