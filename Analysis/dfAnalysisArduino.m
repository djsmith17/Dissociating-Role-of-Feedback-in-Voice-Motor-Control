...
arAn.AnaType   = 'Arduino';
arAn.expType   = expParam.expType;
arAn.subject   = expParam.subject;
arAn.run       = expParam.run;
arAn.curSess   = expParam.curSess;
arAn.f0Type    = 'Praat';
arAn.f0AnaFile = [arAn.subject arAn.run 'f0Analysis.mat'];
arAn.gender    = expParam.gender;
arAn.AudFB     = expParam.AudFB;
arAn.AudFBSw   = expParam.AudFBSw;
arAn.bTf0b     = f0b;

if isfield(expParam, 'balloon')
    arAn.balloon = expParam.balloon;
else
    arAn.balloon = 'N/A';
end
...
[arAn.ContTrials, arAn.contIdx] = find(arAn.trialType == 0);
[arAn.PertTrials, arAn.pertIdx] = find(arAn.trialType == 1);
...
arAn.audioMAl = arAn.audioM(arAn.idxPres(:,1):end, :);

%The Audio Analysis
arAn = dfAnalysisAudio(dirs, arAn, AudFlag, iRF);
    
lims  = identifyLimits(arAn);
